...
// export interface MyAnswerTreeNode {
// 	readonly id: string;
// 	readonly treeItem: TreeItem;
//
// 	getChildren(): MyAnswerTreeNode[] | Promise<MyAnswerTreeNode[]>;
// }
...
export class AnswerCommit {
	...
	readonly sha: string;
	// This is readonly, but must be set by parent node
	parentSha?: string;
	readonly conclusion?: string;
	readonly time: string;

	public fillFromJSON(jsonObj: any): AnswerCommit {
	...
	async getChildren(): Promise<TreeItem[]> {
		console.log(`Try to get changed files for ${this.sha}`);
...
export class PullRequestAnswer {
	...
	constructor(commits: AnswerCommit[]) {
		this.commits = commits;
	}

	static fromJSON(jsonObj: any): PullRequestAnswer {
		const commits = jsonObj['commits'].map((commit) => {
			return new AnswerCommit().fillFromJSON(commit);
		});
		for (let i = 1; i < commits.length; ++i) {
			commits[i].parentSha = commits[i - 1].sha;
		}
		const ret = new PullRequestAnswer(commits);
		for (const propName in jsonObj) {
			if (propName !== 'commits') {
				ret[propName] = jsonObj[propName];
			}
		}
		return ret;
	}
	...
	get htmlUrl() {
		return `https://github.com/${this.repoFullName}/pull/${this.number}`;
	}
